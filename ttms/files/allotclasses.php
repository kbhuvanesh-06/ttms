...
<form action="allotclasses.php" method="post" style="margin-top: 100px">
    ...
    <div align="center">
        <select name="course" class="list-group-item">
            ...
            ';
            <option value="2">BBACA 1ST Year</option>
            <option value="3">BBACA 2ND Year</option>
            <option value="4">BBACA 3RD Year</option>
        </select>
    </div>
...
<div align="center">
    ...
    <table id=allotedclassroomstable>
        ...
        $q = mysqli_query(mysqli_connect("localhost", "root", "2004", "ttms"),
            "SELECT * FROM classrooms ");
        $courses = array('BBACA 1st Year', 'BBACA 2nd Year', 'BBACA 3rd Year');
        while ($row = mysqli_fetch_assoc($q)) {
            if ($row['status'] == 0)
                continue;
